...
public class TenantAppService_Tests : AbpTenantsApplicationTestBase
{
    ...
    public async Task GetListAsync()
    {
        ...
        result.Items.ShouldContain(t => t.Name == "secyud");
    }

    [Fact]
    public async Task GetListAsync_Filtered()
    {
        var result = await _tenantAppService.GetListAsync(new GetTenantsInput { Filter = "secyud" });
        result.TotalCount.ShouldBeGreaterThan(0);
    ...
    public async Task GetListAsync_Sorted_Descending_By_Name()
    {
        ...
        tenants.ShouldContain(t => t.Name == "secyud");

        tenants.FindIndex(t => t.Name == "acme").ShouldBeGreaterThan(tenants.FindIndex(t => t.Name == "secyud"));
    }
    ...
    public async Task UpdateAsync_Should_Not_Allow_Duplicate_Names()
    {
        ...
        await Assert.ThrowsAsync<BusinessException>(async () =>
        {
            await _tenantAppService.UpdateAsync(acme.Id, new TenantUpdateDto { Name = "secyud" });
        });
    }
