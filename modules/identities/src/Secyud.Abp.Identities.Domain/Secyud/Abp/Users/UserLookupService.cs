...
public abstract class UserLookupService<TUser, TUserRepository>(
    ...
    IUnitOfWorkManager unitOfWorkManager) : IUserLookupService<TUser>, ITransientDependency
    ...
    where TUserRepository : IUserRepository<TUser>
{
    ...
    public ILogger<UserLookupService<TUser, TUserRepository>> Logger { get; set; } = NullLogger<UserLookupService<TUser, TUserRepository>>.Instance;

    private readonly TUserRepository _userRepository = userRepository;
    ...
    public async Task<TUser?> FindByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        if (localUser is IUpdateUserData updateUserData && updateUserData.Update(externalUser))
        {
