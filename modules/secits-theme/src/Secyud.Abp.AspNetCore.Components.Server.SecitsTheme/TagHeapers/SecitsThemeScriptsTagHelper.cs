...
[HtmlTargetElement("secits-theme-script", TagStructure = TagStructure.NormalOrSelfClosing)]
public class SecitsThemeScriptsTagHelper : AbpTagHelper
{
    ...
    public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        var styleOptions = ViewContext.HttpContext.RequestServices.GetRequiredService<IOptions<SecitsScriptsOptions>>().Value;
    ...
    private static string GetLinkHtml(IUrlHelper urlHelper, HtmlPathResource resource)
    {
        ...
        return $"""<script src="{url}" id="{resource.Id}"></script>{Environment.NewLine}""";
    }
}