    ...
    : IFeatureManager, ISingletonDependency
{
    ...
    protected List<IFeaturesProvider> Providers => GetFeaturesProvider();
    ...
    private List<IFeaturesProvider> GetFeaturesProvider()
    {
        var uow = UowManager.Current!;
        return uow.GetOrAddItem("SettingsProviders", _ => Options
            .Providers
            .Select(c => (IFeaturesProvider)uow.ServiceProvider.GetRequiredService(c))
            .ToList());
    }
