...
public class FeaturesStore(
    ...
    IDistributedCache<FeatureValueCacheItem, FeatureValueCacheKey> cache,
    IFeatureDefinitionManager featureDefinitionManager)
    : IFeaturesStore, ITransientDependency
{
    ...
    protected IFeatureValueRepository FeatureValueRepository { get; } = featureValueRepository;
    ...
    public virtual async Task SetAsync(string name, string? value, string? providerName, string? providerKey)
    {
        ...
        await Cache.SetAsync(new FeatureValueCacheKey(name, providerName, providerKey), new FeatureValueCacheItem(featureValue.Value), considerUow: true);
    }
    ...
    public virtual async Task DeleteAsync(string name, string? providerName, string? providerKey)
    {
        ...
        foreach (var featureValue in featureValues)
        {
            ...
            await Cache.RemoveAsync(new FeatureValueCacheKey(name, providerName, providerKey), considerUow: true);
        }
    }

    protected virtual async Task<FeatureValueCacheItem> GetCacheItemAsync(string name, string? providerName, string? providerKey)
    {
        var cacheKey = new FeatureValueCacheKey(name, providerName, providerKey);
        ...
        FeatureValueCacheItem currentCacheItem)
    {
        ...
        var cacheItems = new List<KeyValuePair<FeatureValueCacheKey, FeatureValueCacheItem>>();
        ...
        foreach (var featureDefinition in featureDefinitions)
        {
            ...
            cacheItems.Add(
                new KeyValuePair<FeatureValueCacheKey, FeatureValueCacheItem>(
                    new FeatureValueCacheKey(featureDefinition.Name, providerName, providerKey),
