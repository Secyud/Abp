...
{
    [Inject]
    protected IFeatureAppService FeatureAppService { get; set; } = null!;

    [Inject]
    protected IUiMessageService UiMessageService { get; set; } = null!;

    [Inject]
    protected IStringLocalizerFactory HtmlLocalizerFactory { get; set; } = null!;

    [Inject]
    protected IOptions<AbpLocalizationOptions> LocalizationOptions { get; set; } = null!;
    ...
    protected virtual void SetTab(Tab<FeatureGroupDto> tab)
    {
        tab.Key = GetNormalizedGroupName(tab.Item.Name);
    }
    ...
    protected virtual async Task DeleteAsync(string? providerName, string? providerKey = null)
    {
        try
        {
    ...
    protected virtual string? GetShownName(FeatureDto featureDto)
    {
    ...
    protected virtual async Task OnFeatureValueChangedAsync(string? value, FeatureDto feature)
    {
        ...
        {
            await UiMessageService.Warn(L["Secyud.Abp.Features:InvalidFeatureValue", feature.DisplayName!]);
        }
    }
    ...
    protected virtual string? GetDisplayText(LocalizableStringInfo str)
    {
        if (CreateStringLocalizer(str.ResourceName) is { } localizer)
        {
            return localizer.GetString(str.Name);
        }
